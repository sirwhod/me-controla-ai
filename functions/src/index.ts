...
async function runFinancialReminders() {
  ...
  for (const workspace of workspaces.docs) {
    ...
    for (const debit of debits.docs) { const d = debit.data(); if (d.isTemplate || d.status === 'paid' || d.type !== 'Fixo') continue; const dueDay = Number(d.dueDay) || Number(dateOnly(d.dueDate)?.getDate()); if (!dueDay) continue; const delta = dueDay - now.getDate(); for (const userId of users) { if (delta === 1) await notifyOnce(userId, `debit-due:${debit.id}:${today}`, 'Conta vencendo amanhã', `${d.description || 'Uma conta'} no valor de R$ ${Number(d.value || 0).toFixed(2)} vence amanhã.`, `/${workspace.id}/dashboard/debits`); if (delta === 0) await notifyOnce(userId, `debit-today:${debit.id}:${today}`, 'Conta vence hoje', `${d.description || 'Uma conta'} vence hoje.`, `/${workspace.id}/dashboard/debits`) } }
    for (const card of cards.docs) { const c = card.data(); const due = Number(c.dueDay); const closing = Number(c.closingDay); if (!due) continue; const day = now.getDate(); const currentMonth = now.toLocaleString('pt-BR', { month: 'long' }); const cardTotal = debits.docs.filter((debit) => { const d = debit.data(); return d.creditCardId === card.id && d.status !== 'paid' && !d.isTemplate && d.year === now.getFullYear() && d.month === currentMonth }).reduce((total, debit) => total + Number(debit.data().value || 0), 0); for (const userId of users) { if (closing && day === closing) await notifyOnce(userId, `card-close:${card.id}:${today}`, 'Fatura fechada', `A fatura do cartão “${c.name || 'Cartão'}” foi fechada.`, `/${workspace.id}/dashboard`); if (day === due) await notifyOnce(userId, `card-due:${card.id}:${today}`, 'Fatura vence hoje', `A fatura do cartão “${c.name || 'Cartão'}” vence hoje no valor total de R$ ${cardTotal.toFixed(2)}.`, `/${workspace.id}/dashboard`) } }
