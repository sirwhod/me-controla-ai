...
export const columns: ColumnDef<Debit>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const getTypeIcon = () => {
        switch (type) {
          case "Comum":
            return <BanknoteArrowDown className="w-4 h-4 text-muted-foreground" />
          case "Fixo":
            return <Pin className="w-4 h-4 text-muted-foreground" />
          case "Parcelamento":
            return <CreditCard className="w-4 h-4 text-muted-foreground" />
          case "Assinatura":
            return <CalendarSync className="w-4 h-4 text-muted-foreground" />
          default:
            return null
        }
      }
  ...
  },
  {
    ...
    header: ({ column }) => {
      return (
        <DataTableColumnHeader column={column} title="Banco / Conta" />
      )
    },
    cell: ({ row }) => {
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-2 font-medium">
            {bankImageUrl ? (
              ...
              <div className="bg-primary/10 text-primary p-0.5 rounded-xs">
