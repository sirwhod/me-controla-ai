...
import { ResponsibleForm } from "@/app/components/forms/responsible-form"
...
export default function EditResponsiblePage() {
  ...
  const responsibleId = params?.responsibleId as string
  ...
  const {
    ...
    queryKey: ["responsible-details", effectiveWorkspaceId, responsibleId],
    queryFn: () => getResponsibleDetails(effectiveWorkspaceId, responsibleId),
    enabled: !!effectiveWorkspaceId && !!responsibleId,
  })

  const isLoading = (isWorkspaceLoading && !workspaceActive) || isResponsibleLoading
        ...
        <div className="flex items-center gap-2 w-full">
          ...
          <Breadcrumb>
            <BreadcrumbList className="text-xs sm:text-sm">
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link href={`${effectiveWorkspaceId ? `/${effectiveWorkspaceId}` : ""}/manage/responsibles`}>
                    Responsáveis
                  </Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
      ...
      <main className="flex-1 p-3 md:p-6 pt-3 max-w-2xl w-full mx-auto pb-20 md:pb-8">
        ...
        {isLoading ? (
          ...
          <ErrorState
            ...
            message={responsibleError?.message || "Não foi possível carregar as informações deste responsável."}
            onRetry={() => refetch()}
          />
        ) : (
          <ResponsibleForm mode="edit" responsible={responsible} />
        )}
      </main>
    </div>
  )
